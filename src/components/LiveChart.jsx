...
export function useCandles(symbol, tf, limit = 500) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!dead) setS({ candles: data.candles, data: data.data, hasVolume: data.has_volume, loading: false, error: null, key, structure: data.structure });
      } catch (e) {
        if (!dead) setS((x) => ({ ...x, loading: false, error: errText(e) }));
      }
    };
    run();
...
export default function LiveChart({ tf, layers, heightClass, limit = 500 }) {
  const { prefs, live, history } = useLive();
  const { candles, hasVolume, loading, error, structure } = useCandles(prefs.symbol, tf, limit);
  ...
  return (
    <div className="relative h-full">
      <Chart candles={candles} analysis={live?.analysis} own={structure} tf={tf} layers={layers} marks={marks} precision={info.precision}
        barSeconds={tfSeconds(tf)} viewKey={`${prefs.symbol}|${tf}`} hasVolume={hasVolume} heightClass={heightClass} />
